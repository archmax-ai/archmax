...
Use after a burst of feature work when you suspect accumulated cruft: dead code, duplicated logic, oversized files, or misplaced responsibilities. For changes that alter behaviour, add capabilities, or break APIs, use `/opsx:propose` instead.
...
1. **Identify candidates.** Scan across `packages/core/src/`, `apps/api/src/`, `apps/frontend/src/`, and `apps/worker/src/` for:
   ...
   - **Inconsistent patterns** — places that deviate from project conventions (see the `context:` block in `openspec/config.yaml`): raw `throw` instead of `AppError`, class components, missing Zod validation, `require()` in ESM code.
