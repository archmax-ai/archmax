...
description: Set up a local development environment for contributing to archmax semantics.
---
...
4. **Start development servers**
   ...
   - The API trusts only `http://localhost:5173` as a browser origin, so the dev server runs with `strictPort` and fails if 5173 is taken instead of moving to a port the API rejects with `Invalid origin`. To use another port, set `CORS_ORIGINS=http://localhost:<port>` in `.env.local` and start the frontend with `pnpm --filter @archmax/frontend dev -- --port <port>`.
...
In CI, E2E tests run automatically after the Docker image is built on every pull request. That image is published as `ghcr.io/archmax-ai/semantics:pr-<number>` (the image name is fixed in the workflows and independent of the repository name), so you can run the same stack locally against a PR build with `APP_IMAGE=ghcr.io/archmax-ai/semantics:pr-<number>`.
