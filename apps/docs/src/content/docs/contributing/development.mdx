...
4. **Start development servers**
   ...
   - If 5173 is taken (another project's dev server, say), set `FRONTEND_PORT=<port>` in the repo-root `.env.local`. Vite listens there and the API trusts `http://localhost:<port>` as a browser origin; without it, Vite fails fast on a busy port rather than moving to one the API would reject with `Invalid origin`.
