...
archmax semantics uses [DuckDB](https://duckdb.org/) as an in-process query engine that can federate queries across multiple database connections. Each project gets its own DuckDB instance with connections attached as named schemas.

## How It Works

When you add a connection to a project and mark it as active, archmax semantics attaches it to the project's DuckDB instance using DuckDB's extension system:
...
When using **Connection Details** (host/port fields), archmax semantics builds an ADO.NET connection string:
...
Unlike RDBMS connections, Iceberg uses a two-step attach: archmax semantics creates a DuckDB secret with the bearer token, then attaches the catalog with the REST endpoint.
...
- **Side-channels through the persistent file.** `<SEMANTICS_DATA_DIR>/projects/<projectId>/duckdb.db` holds every model's `_scope_<model>` schema in plain DDL. Anyone who can read the file can read every materialised view body. Protect the data directory with the same filesystem permissions you would give a database data dir.
...
- **Don't put two trust domains in the same project.** Separate customers, separate environments (prod vs staging), or separate sensitivity tiers go into separate archmax semantics projects so they don't share the DuckDB instance, the persistent file, or the connection set.
...
- **Lock down `SEMANTICS_DATA_DIR`** with filesystem-level permissions; treat each project's `duckdb.db` as sensitive.
...
Each project's DuckDB state lives in a persistent `duckdb.db` file under `<SEMANTICS_DATA_DIR>/projects/<projectId>/`. The instance is opened lazily on first use and closed on graceful shutdown so the file lock is released cleanly. Materialised VIEWs survive restarts because they are stored in that file. The file MUST NOT be committed to a project's Git repo (it is in the default project `.gitignore`) and it is safe to delete when the project is offline.

## Persistent DuckDB File (Operations)

Each project's DuckDB instance is backed by a single file at `<SEMANTICS_DATA_DIR>/projects/<projectId>/duckdb.db` (plus its `.wal` and `.tmp` siblings during writes). Materialised VIEWs survive process restarts because they are stored in this file.

Operational notes:

- **Where it lives** — `<SEMANTICS_DATA_DIR>/projects/<projectId>/duckdb.db`. `SEMANTICS_DATA_DIR` defaults to `./data` in development.
