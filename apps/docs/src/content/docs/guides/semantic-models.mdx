...
archmax semantics uses the OSI YAML format as its internal storage format — every dataset, field, relationship, and metric is persisted as spec-compliant YAML on disk. When AI agents request model information through MCP tools, archmax semantics converts the OSI YAML into a **compressed markdown digest** on-the-fly. This digest preserves all semantically relevant information (field types, descriptions, enums, relationships, examples) while using **3–5× fewer tokens** than the equivalent YAML, making agent interactions significantly cheaper and faster.

## File Structure

Each project stores its semantic models under `<SEMANTICS_DATA_DIR>/projects/<projectId>/src/`. A model consists of a root file and per-dataset files:
