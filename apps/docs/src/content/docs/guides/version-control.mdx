...
description: How archmax semantics tracks changes to your semantic models with built-in Git and optional GitHub sync.
---

Every archmax semantics project has a built-in Git repository that automatically tracks changes to your semantic models. You can optionally connect a GitHub remote to keep an external copy in sync.

## How It Works

archmax semantics initializes a local Git repository inside each project's data directory. Every time you **publish** your semantic models, archmax:
...
archmax semantics creates a `.gitignore` with sensible defaults:
...
The **Publish** button appears in the UI whenever archmax semantics detects changes in the project directory. Change detection works by computing a SHA-256 hash of all files in the project root, excluding:
...
Each history entry (except the most recent) shows an **undo icon**. Clicking it opens a confirmation dialog — after confirming, archmax semantics:
...
archmax semantics uses [isomorphic-git](https://isomorphic-git.org/) for all Git operations. When pulling from a remote:
