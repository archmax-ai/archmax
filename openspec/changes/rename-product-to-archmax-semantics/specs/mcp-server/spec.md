...
The server name is advertised during the MCP initialize handshake only. Existing bearer tokens, project slugs, and endpoint URLs are unaffected by the rename, so an already-configured client continues to authenticate and call tools without changes.
...
- **THEN** the server identifies itself with name `"archmax-semantics"`
- **AND** documentation examples show `mcpServers["archmax-semantics"]` as the configuration key
- **AND** the legacy `"archmax"` server name is no longer advertised

#### Scenario: Existing client keeps working after the rename

- **WHEN** a client configured before the rename connects with its existing bearer token and endpoint URL
- **THEN** authentication succeeds
- **AND** all tools respond as before
- **AND** only the advertised server name differs
