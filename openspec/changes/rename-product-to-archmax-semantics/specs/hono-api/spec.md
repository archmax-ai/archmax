...
- Data directory writability (`SEMANTICS_DATA_DIR`)
...
- **AND** `APP_BASE_URL` is set to `https://semantics.example.com`
- **AND** a request arrives from `https://semantics.example.com`
- **THEN** appropriate CORS headers are returned
