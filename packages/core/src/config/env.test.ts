...
describe("env — APP_BASE_URL derivation", () => {
  ...
  it("derives both AUTH_BASE_URL and corsOrigins from APP_BASE_URL", async () => {
    ...
    expect(env.AUTH_BASE_URL).toBe("https://semantics.example.com");
    expect(env.corsOrigins).toEqual(["https://semantics.example.com"]);
  });

  it("explicit CORS_ORIGINS overrides APP_BASE_URL", async () => {
    process.env.APP_BASE_URL = "https://semantics.example.com";
    process.env.CORS_ORIGINS =
      "https://semantics.example.com,https://other.example.com";
    delete process.env.AUTH_BASE_URL;
